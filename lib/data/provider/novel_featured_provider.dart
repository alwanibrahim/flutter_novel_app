...
class NovelFeaturedProvider with ChangeNotifier {
  ...
  List<NovelModel> get latestNovels {
    List<NovelModel> sorted = [..._dataListfeatured];
    sorted
  ...
  Future<void> getNovel() async {
    ...
    try {
      _dataListfeatured = await _apiService.fetchData();
      _error = null;
    } catch (e) {
      _error = e.toString();
    }
  ...
  Future<void> getNovelFeatured() async {
    ...
    try {
      _dataListfeatured = await _apiService.getFeaturedNovels();
      _error = null;
    } catch (e) {
      _error = e.toString();
    }
  ...
  Future<List<NovelModel>> getFetchedNovels() async {
    ...
    return _dataListfeatured; // Kembalikan data yang sudah di-fetch
  }
  ...
  void searchNovels(String query) {
    ...
    _searchResults = _dataListfeatured.where((novel) {
      ...
      final categoryMatch =
          novel.category.name.toLowerCase().contains(lowerQuery);
  ...
  Future<void> refresh([int? novelId]) async {
    await getFetchedNovels();
    await getNovel();
    if (novelId != null) {
      await getNovelDetail(novelId);
    }
  }
