...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() {
      Provider.of<NovelProvider>(context, listen: false).getNovel();
    });
  }
...
Widget _buildFeaturedNovels(BuildContext context) {
    final noveProvider = Provider.of<NovelProvider>(context);

    if (noveProvider.isLoading) {
    ...
    if (noveProvider.error != null) {
      return SizedBox(
        height: 180,
        child:
            Center(child: Text('Terjadi error: ${noveProvider.error}')),
      );
    }

    if (noveProvider.dataList.isEmpty) {
      return const SizedBox(
        height: 180,
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemCount: noveProvider.dataList.length,
        padding: const EdgeInsets.symmetric(horizontal: 10),
        physics: const BouncingScrollPhysics(),
        itemBuilder: (context, index) {
          final novel = noveProvider.dataList[index];
...
// Helper function to get status label
  ...
  Widget _buildLatestNovels(BuildContext context) {
    // Sort novels by publication date (newest first)
    final latestNovels = List<Novel>.from(Novel.novels)
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final novel = latestNovels[index];
        return GestureDetector(
          onTap: () {
            Navigator.push(
              context,
